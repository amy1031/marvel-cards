function CardsController() {
  ...
  this.onAdd = function (id) {
    ...
    dataStore.addToPlayerCharacters(id);
    draw(dataStore.getMarvelCharacters(), dataStore.getPlayerCharacters());
    }

  this.onRemove = function (id) {
    //this will remove the character from your team
    dataStore.removePlayerCharacter(id);
    draw(dataStore.getMarvelCharacters(), dataStore.getPlayerCharacters());
  }

  this.createPlayer = function createPlayer(e) {
    e.preventDefault();
    var form = e.target;
    dataStore.createPlayer(form.name.value)
  //  draw(dataStore.getMarvelCharacters(), dataStore.getPlayerCharacters());
  }
  ...
  function draw(marvelList, myList) {
    ...
    myElem.innerHTML = playerTemplate1;
